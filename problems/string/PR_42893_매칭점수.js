function solution(word, pages) {
  ...
  pages.forEach((page, idx) => {
      ...
      const basic_score = body.flatMap(str => str.split(REG_WORD)).filter(str => str.toLowerCase() === word).length
      
      // 배열 중에 null이 아닌 친구들만 고르기 .filter(e => e)
      // string을 index 별로 자르기 string.substr(startIdx, endIdx), endIdx는 포함 안 함
      const outer_links = body.flatMap(str => str.match(REG_URL)).filter(e => e).map(str => str.substr(8, str.length).split('>')[0])
